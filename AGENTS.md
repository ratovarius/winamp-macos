...
`Sources/` is a single SPM/Xcode module. Use a language server, `grep`, or the file tree to find a
given file — this section is intentionally *not* a file listing (those go stale and the tooling
answers "where is X?" better). It documents each area's **responsibility and the rules to respect**:
...
> **Adding files:** both the `Sources/` and `Tests/WinampTests/` groups are Xcode
> **filesystem-synchronized root groups**, so a new `.swift` file under either compiles
> automatically — **no `project.pbxproj` edits** (SPM already globs `Sources/`). Just create it and
> build. Exception: `Tests/Fixtures/` (generated by `generate-fixtures.sh`, gitignored) stays an
> *explicit* group and is deliberately **not** synchronized, so generated files aren't swept into
> the build.
